...
description: 为目标项目受控接入、升级、修复或维护 CodeGraph 与 RTK；在写入项目配置、安装受管版本、重建索引、健康检查或回滚时使用。
---
...
仅在用户明确要求以下状态变更或维护时使用：项目接入、安装、升级到受支持版本、配置修复、索引初始化或修复、健康检查或回滚。不要因发现 CodeGraph/RTK 可用于当前开发任务而自行接入、升级或维护。
...
5. 接入完成后初始化或同步索引并运行完整 `doctor`。CodeGraph MCP 配置通常需要新建 Codex task 或重启客户端才能加载。
6. 只有明确的故障、健康检查、索引异常或回滚任务才运行诊断与维护命令；不要为普通开发定期检查或同步。

## 按需资料

- 首次接入、安装、升级或修复缺失工具：读取 [`references/install.md`](references/install.md)。
