...
每个阶段返回 `HandoffPacket`，由协调者显式交给下游；role 间不假设隐式对话连通。核心字段是：`work_id`、目标与范围/非目标、状态、输入或产物引用、可验证结果与证据、`guard_results`、未覆盖行为、升级原因、风险/未知项和下一阶段请求。涉及写入或并发时额外提供 `ImplementationContract`、依赖、允许目标或所有权、验收、停止条件和集成负责人。默认使用自包含任务包与相关产物引用；只有会改变判断的近期历史才附加。下游缺少影响判断的输入时交回父角色，不猜测。
