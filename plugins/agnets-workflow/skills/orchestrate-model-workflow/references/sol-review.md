...
CLI 的子进程 stdout/stderr 是机器审核证据：只允许写入受控 stdout/stderr 制品并由解析器消费，不得 tee 到调用方终端或聊天文本。CLI 外层 stdout 只输出自然语言摘要，完整 outcome 只保存在全局受控制品；MCP 调用应优先消费 `structuredContent`，`content.text` 只放人类摘要。
