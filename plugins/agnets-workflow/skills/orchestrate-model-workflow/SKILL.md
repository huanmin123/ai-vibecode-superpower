...
| 最高强度复审与最终升级验收 | `avsp_sol_max` | `avsp_sol_xhigh` 修复后仍 `fail` 时使用；首次进入冻结可审计 charter，完成一次受保护批量修复后进行受控 closure review；只有 charter closure `pass` 才可关闭 |
...
全部非审核节点完成后，main/root 汇总验证、清理已知派生产物并冻结当前快照与工作区指纹；这时才启动清单中唯一的末端质量门。门通过才可关闭、释放租约和交付；门失败后的修复回到任务内部，修复完成后重新进入同一个末端门。审核节点必须直接依赖全部非审核节点，任何节点不得依赖审核节点，禁止为单点、小任务或节点交接单独追加审核。执行期间若新证据证明原门级不足，只能在末端门被认领前调用 `workflow_raise_assurance` 单调提高一次或多次等级：`verification -> terra/sol`、`terra -> sol`；该入口只新增或转换唯一末端审核节点，不允许通用追加节点或降级。

### 默认分流

跨文件、未知根因或需扫描的任务，若能拆出独立只读证据域，默认先派 `avsp_luna_high`；单文件且改法明确时可跳过并记录理由。主控保留设计、授权、集成和最终判断。

`routing_schema_version=1` 清单保持原有严格行为：非总审 `read_only`/`delegable` 分别默认到 `avsp_luna_high`/`avsp_luna_high_executor`，唯一 `total_review` 默认到 `avsp_sol_high`。新任务使用 `routing_schema_version=2`，必须声明抽象的 `assurance_level`（`verification`、`terra` 或 `sol`）以及 `assurance_assessment`。`impact`、`recoverability`、`uncertainty`、`verifiability`、`coupling` 各自必须包含 `status`、非空 `evidence` 数组和 `rationale`，`status` 只取 `controlled`、`partial` 或 `unknown`，另带非空 `selection_reason`；不得用业务场景白名单代替这些维度。全部维度为 `controlled` 时门级必须是 `verification`，任一为 `partial` 且没有 `unknown` 时必须是 `terra`，任一为 `unknown` 时必须是 `sol`。`terra` 从末端 `quality_review` 和 `avsp_terra_xhigh` 开始，`sol` 直接使用末端 `total_review`。显式 `agent_type` 必须与 `execution_risk` 兼容；控制器在初始化时拒绝会导致节点无法认领的组合。Sol 仅在所选 Sol 实际不可用时使用 Terra fallback。共享写入、外部副作用或不可恢复变更仍为 `protected`，由 Terra 处理。
...
2. **需要判断**：证据充分时，`avsp_terra_xhigh` 与 `avsp_sol_high` 同属定案层，按难度分流而非先后升级：范围有界且推理路径常规时选 Terra；存在多约束权衡、跨域影响或高复杂度推理时选 Sol。高影响或风险域名称本身不触发 Sol。审核 `fail` 必须返回至少一个结构化 blocking finding，包含唯一 `id`、`severity`、可空 `requirement_id`、`summary` 和 `evidence`。v2 的末端 `terra` 门只允许两次有效 Terra 审核：一审 `fail` 后，main/root 在任务内部修复明确问题并调用 `workflow_record_repair`，让 `addressed_findings` 逐项绑定并覆盖全部 blocking finding，才可由新的 Terra 二审；修复记录可以对应代码修复、补充验证证据或外部状态修复，控制器会记录 `workspace_changed`，不以文件指纹必须变化为前提；二审仍 `fail` 时不再局部修复，直接把当前快照、修复记录和两轮证据交给新的 Sol 全面定案。进入 Sol 后使用单调升级链：第一次 `avsp_sol_high` `fail` 后修复并仍由新的 high 实例复审一次；第二次连续 high `fail` 后下一轮自动切换 `avsp_sol_xhigh`；xhigh 在修复后再次 `fail` 时冻结 `max_review_charter`，其中包含当前快照/指纹、原始验收 requirements、`workspace_claims`、固定 blocking finding IDs 与受控 closure 次数。Terra 必须先以一次受保护修复覆盖这批 charter blockers，再由新的 `avsp_sol_max` 只作 closure review。closure 复核 charter findings、受影响 diff/回归和原始需求；新 finding 只有能逐项证明为本次 repair 引入且关联原 requirement（`repair_regressions`）时才可进入当前 charter 的一次追加修复。其他新 blocker 记录为 `out_of_charter` 并将任务置为 `scope_decision_required`/`blocked`：不得自动重开、不得当作 pass，main/root 或用户必须选择替代任务或显式扩大 charter。只有 closure `pass` 才可关闭；旧或缺 charter 的 max 状态 fail-closed。`unavailable`、超时、证据不足或无效输出不计入 Terra 或 Sol 的失败次数，也不占用普通执行重试预算；它们使用独立的有限不可用预算，只在确认旧实例停止后按当前等级重试或补证。
3. **需要状态变更**：先写完整 `execution_contract` 和五项路由字段；仍有行为、风险或兼容性选择时不得委派 executor。main/root 直派时负责集成，Terra 直派时负责审核和集成。
4. **执行风险**：可证明没有任何工作区、数据或外部状态变更的取证为 `read_only`，交给只读 role；只有影响面受控、失败可回滚或恢复、外部副作用受限、共享状态与顺序风险已解决且有可靠验证的写入才是 `delegable`；其余写入为 `protected`，由 Terra 直接执行。main/root 或 Terra 可把 `1..N` 个互不冲突的 `delegable` 契约并行交给 Luna executor；默认 Luna/high，只有完整契约仍需更深局部理解时用 xhigh executor，并在 `routing_reason` 记录理由。使用控制器时，将写入/审阅范围写成不可变 `workspace_claims`；claims 是协调声明而非 ACL，范围扩大必须确认旧执行者停止并释放 entry 后，以新 `task_id` 和 claims 超集创建替代任务。缺少任一审计字段、字段冲突、旧清单或共享写入时一律按 `protected`，不得静默当作可委派。`execution_owner` 是每个状态变更的唯一执行者；并行任务的写入目标必须互斥，共享或全局副作用使用根目录 `write` 独占。
5. **质量门与关闭**：每个任务只有一个末端质量门，不能按节点拆分。v1 的完整任务继续使用独立 Sol 总审。v2 的 `verification` 必须由 main/root 在全部工作节点完成后写入覆盖全部需求、匹配当前 `workflow_snapshot` 和工作区指纹的结构化验证记录；`terra` 必须由每次均为新实例的 `avsp_terra_xhigh` 审核，按上述一审修复、二审直升 Sol 的规则执行；`sol` 从一开始就由新的独立 Sol 审核。执行期间升级后的门仍是同一个任务级末端门。Sol 必须核验原始目标、验收条件、范围与非目标、执行契约、实际 diff/产物、验证结果、需求覆盖、范围漂移、回归风险和未验证项，并确认实际 diff/产物均属于任务的 `write` claims；无法证明时不得 pass。所有门级都要求当前快照和 scoped 指纹匹配；范围外 peer 变化不使已有 `pass` 失效，但范围内与 read claim 变化会使其失效。max 还要求已冻结的 charter 为 `closure_passed`；`scope_decision_required`、缺 charter 或未完成受保护 repair 一律拒绝关闭。关闭检查发现这种失效时，main/root 必须调用 `workflow_invalidate_gate` 清除旧验证记录，或以新的独立 reviewer 路径重开同一个审核节点，再基于新快照完成末端门；不得复用旧 `pass`，也不得新建第二个审核节点。`fail` 或 `unavailable` 时不得关闭任务，main/root 必须确认旧实例停止后派遣修复、补证或同级重试。进入 Sol 后仅所选 Sol role 或模型被实际证明不可用时，才使用此前未参与该任务的 `avsp_terra_xhigh_readonly` 兜底并披露独立性下降；超时、证据不足或普通失败不得降级。旧状态若已把未记录关卡标为成功，确认旧实例已停止后才可显式重开该节点，不得伪造或补写旧记录。
