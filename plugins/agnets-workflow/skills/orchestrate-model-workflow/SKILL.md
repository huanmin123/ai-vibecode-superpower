...
1. **只需事实**：按独立且互补的证据域直接委派 `1..N` 个 Luna，并在控制器清单中设 `execution_risk=read_only`；不得为同一事实重复创建分支，也不得为让 Luna 可用而写成 `delegable`。证据不足但可补充时继续 Luna；无法补证、授权缺失或必要外部输入缺失时停止并报告。
