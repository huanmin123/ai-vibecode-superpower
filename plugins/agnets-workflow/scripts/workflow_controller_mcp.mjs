...
export const TOOLS = [
  ...
  ['workflow_retry', '确认旧执行者停止后，将 failed、blocked、unavailable 或 abandoned 节点重新置为待认领。', ['task_id', 'node_id', 'reason', 'previous_agent_stopped', 'state_dir'], { task_id: { type: 'string' }, node_id: { type: 'string' }, reason: { type: 'string' }, previous_agent_stopped: { type: 'boolean', const: true }, state_dir: { type: 'string' } }],
  ['workflow_requeue_stale', '协调者已用原生状态确认旧代理停止后，原子地重排队已过期 claim，并返回给新代理的持久恢复包；控制器不能自行停止或恢复 Codex 代理。', ['task_id', 'node_id', 'claim_id', 'reason', 'previous_agent_stopped', 'state_dir'], { task_id: { type: 'string' }, node_id: { type: 'string' }, claim_id: { type: 'string' }, reason: { type: 'string' }, previous_agent_stopped: { type: 'boolean', const: true }, state_dir: { type: 'string' } }],
  ...
  ['workflow_prune_expired', '删除 state_dir 内连续 7 天未更新、租约已释放且没有运行节点的任务状态；不会删除活动、未知或无法解析的任务。每个有 state_dir 的工作流操作也会执行同一惰性清理。', ['state_dir'], { state_dir: { type: 'string' } }],
