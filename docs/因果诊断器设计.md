...
当前已经具备可运行的只读闭环：`causal_status` 检查 CodeGraph 索引，`causal_analyze` 先用确定性解析器提取事故描述锚点，再在有界范围内合并代码关系、日志/堆栈证据、RTK 压缩结果、多假设评分和反事实字段，并通过本仓库 marketplace 与平台安装器提供给 Codex。它的目标是让模型一次获得高密度、可追溯的全局证据，而不是替模型保证根因正确。
...
当前实现不重新扫描源码来重建通用调用图或维护第二套函数级静态索引。它复用项目已经受管安装的 `@colbymchenry/codegraph@1.6.0` 的公开 SDK，并读取同一个项目级 `.codegraph/` 索引。该版本的公开类型声明提供 `CodeGraph.open`、`getNode`、`getIncomingEdges`、`getOutgoingEdges`、`traverse`、`getCallers`、`getCallees`、`getImpactRadius`、`findPath`、`findRelevantContext`、`getIndexState`、`isIndexStale` 和 `getPendingFiles` 等能力。
...
全局受管的 `codegraph` CLI 已能维护项目索引，但 Node 的全局安装位置不是插件进程可安全 `import` 的契约。`causal-debugger` 因此应在自己的 `package.json` 中声明精确的 `@colbymchenry/codegraph@1.6.0` 依赖，并由 lockfile 固定解析结果；它使用该 SDK 打开同一个 `.codegraph/`，不再安装或启动第二套索引器。
...
输入解析按确定性强弱生成 seed。当前解析器在不调用模型的情况下识别文件/行号、错误码、符号、HTTP endpoint、SQL 表标识、配置/环境键和 trace/request/service 键；CodeGraph 只负责验证这些候选是否存在于当前索引，解析器不会自行猜测节点 ID：
...
用户交互使用三态澄清门，避免“每次都追问”和“信息不足仍盲搜”两个极端：

| 状态 | 条件 | 用户可见行为 |
| --- | --- | --- |
| `analyze` | `file:line` 已映射、日志/堆栈已绑定，或调用方明确提供 query/seed/runtime | 直接返回候选和证据，不打断用户 |
| `analyze_and_clarify` | 有错误码、符号、endpoint、配置键等有界锚点，但仍有多条可行路径或图被截断 | 先展示当前发现；仅当缺口会改变下一步时询问一个高信息量问题 |
| `clarify_first` | 没有任何确定性锚点，或 CodeGraph 没有命中可验证节点 | 停止扩散，只请求完整报错/堆栈、失败测试，或最短复现步骤与期望/实际结果 |

澄清问题不能泛泛要求“描述详细一点”，也不能一次索取长清单。默认只问一个条件式问题：优先请求完整报错或堆栈及 `file:line`；如果没有报错，再请求最短复现步骤和期望/实际结果。跨服务候选已经出现时，可改问同一次失败的 trace/request ID。用户已经主动给出 query、seed、日志或运行时事件时，不重复要求其重述问题。
...
当前实现的语言范围等于受管 `@colbymchenry/codegraph@1.6.0` 运行时 `getSupportedLanguages()` 返回的全部语言。适配器不自行识别、解析或补写这些语言；插件启动时读取运行时清单，并按当前 workspace 的实际索引结果报告覆盖范围。
...
受管工具实际名称为 `rtk`（当前版本 `0.46.0`）；`rtk log` 适合压缩文件或标准输入中的重复日志，`rtk pipe` 适合已有过滤器的管道，`rtk rg` 适合先做窄范围匹配。RTK 只是输出压缩器，不理解业务因果，也不保证保留每一行；因此每个压缩片段必须保存原始文件摘要、行号/偏移窗口和 SHA-256。压缩失败、字段解析失败或 RTK 不可用时，保留原始错误并降级为受限窗口读取，不把不完整摘要报告为完整日志事实。
...
“把精华给 AI”不能等价于“按总分删除低分内容”。裁剪必须是从完整证据账本生成一个可替换的视图，原始节点、边、运行时事件、未知项和关系缺口仍由本次 `analysis_id + snapshot` 关联保存。没有稳定账本和展开入口时，不允许把裁剪后的 packet 宣称为完整问题图。

证据选择分为三层：

1. **完整账本层**：保存本次实际读取到的全部规范化证据，以及每条证据的 `id`、来源、快照、文件/行号、原始摘要哈希和脱敏状态。账本可以只读、按需加载，但不能因生成 AI packet 而丢弃。
2. **硬保留层**：任何预算下都优先保留；如果硬保留集合本身超过预算，返回 `budget_exceeded` 或 `needs_clarification`，不得静默删除。
3. **排序裁剪层**：只有硬保留层装入预算后，才按可解释分数选择补充内容。分数只表示阅读优先级，不表示因果概率或事实真值。

硬保留集合至少包括：

- 用户明确提供的文件、行号、堆栈帧、错误码、端点、SQL、配置键和 Trace/Request ID 对应节点；
- 每个 symptom seed 的至少一条最高分可行路径；Top-K 假设的路径节点和路径边；
- 所有运行时事件映射节点、`runtime_observation` 边以及无法映射的事件摘要；
- 所有冲突证据、关系缺口、未覆盖 seed、推荐 probe 和 unknown；
- 连接多个症状或多个候选根因的桥接节点；
- 被保留边的两个端点，保证图没有 dangling edge；
- 路径上的配置、协议、序列化、HTTP、消息、数据库、guard 和测试节点，即使它们的静态分数较低。

可选证据的排序特征应逐条输出，至少包含：直接命中强度、覆盖的 seed/假设数量、是否连接硬保留节点、运行时支持和时间顺序、边的 `evidenceLevel`、静态解析强度、证据类型多样性，以及重复/模板噪声惩罚。选择器必须使用每个 seed 的配额和证据类型配额，避免一个高出度 helper 把其它症状全部挤出 packet。

packet 必须同时返回 `coverage_manifest`，内容包括：

- 本次候选分析图（不是整个仓库）的节点/边数量，以及硬保留数量、实际装入数量和预算；如果上游 CodeGraph 已截断，必须标记 `complete=false`；
- `includedIds`、omitted 数量、omitted ID 列表的哈希和可展开引用；为避免索引本身耗尽 token，omitted ID 列表默认不发送，审计调用才显式请求完整列表；
- 每个 seed 是否保留路径、每个假设是否保留至少一条路径、关系端点闭包是否成立；
- `mandatoryOverflow`、`budgetExceeded`、`truncationReasons`、索引快照、选择器版本和 packet 哈希。

如果 `mandatoryOverflow=true`，默认行为是停止生成可供模型下结论的 packet，并请求提高预算、缩小范围或补充一个高信息量锚点。不能把“只保留 Top 20”当成成功，也不能用一条“已截断”文字掩盖哪些关系没有进入模型上下文。后续的 `causal_expand` 应按 `analysis_id`、`nodeId`、`relationId` 或 seed 定向展开 omitted 内容，并验证快照没有变化。

日志和控制台输出也遵守同一契约。RTK 的去重/折叠只能作用于重复噪声；关键错误、异常堆栈、状态转换、Trace/Request 关联事件、首次和最后一次出现及其前后窗口必须先提取并保留原始行号/偏移和 SHA-256。RTK 失败、解析失败或脱敏失败时保留失败元数据和 unknown，不能把压缩摘要当成完整日志事实。

当前实现状态必须如实区分：`expandNeighborhood` 已有固定节点上限、双向有界 BFS 和 `truncated` 标记，`causal-engine` 已有假设排序和 seed 覆盖统计；`evidence-selection` 已实现硬保留集合、价值感知排序、`coverage_manifest`、端点闭包和按 ID 展开，默认聚焦预算为 64 个节点/128 条边。当前版本可以保证“已进入候选分析图的硬保留关系不会被 packet 裁剪静默删除”，但仍不能保证 CodeGraph 上游有界搜索本身发现整个仓库的所有真实关系；完整候选账本目前随 MCP 进程缓存存在，进程重启后需重新分析。
...
- 固定适配 `@colbymchenry/codegraph@1.6.0` 的公开 SDK，不读取私有 SQLite schema；
