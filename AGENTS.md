...
2. 运行最贴近改动的验证；安装链路改动或全局配置改动至少运行 `pwsh -NoProfile -File .\tests\install-codex.ps1`。
3. 只有用户要求部署时，运行对应平台的 `install-codex` 脚本，并核对已安装文件与源文件一致。
4. 保留实际错误和未验证项；不得用未证明兼容的 fallback 隐藏问题。
