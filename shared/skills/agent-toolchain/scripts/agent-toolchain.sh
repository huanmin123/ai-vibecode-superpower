...
usage() {
  cat <<'EOF'
Usage:
  agent-toolchain.sh configure --project PATH [--client codex|zcode|both]
...
parse_args() {
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --client) [ "$#" -ge 2 ] || die "--client 缺少取值"; CLIENT=$2; shift 2 ;;
      --apply) APPLY=1; shift ;;
  ...
  if [ -n "$CLIENT" ]; then
    case "$CLIENT" in
      codex|zcode|both) ;;
      *) die "未知 --client 取值：$CLIENT（可用：codex|zcode|both）" ;;
    esac
    [ "$ACTION" = configure ] || die '--client 仅适用于 configure'
  fi
}
...
configure_project() {
  ...
  if [ -n "$CLIENT" ]; then
    case "$CLIENT" in
      codex) wire_codex=1 ;;
      zcode) wire_zcode=1 ;;
      both) wire_codex=1; wire_zcode=1 ;;
    esac
  else
    [ -d "$codex_dir" ] && wire_codex=1
    [ -d "$zcode_dir" ] && wire_zcode=1
    if [ "$wire_codex" -eq 0 ] && [ "$wire_zcode" -eq 0 ]; then
      if [ -t 0 ]; then
        printf '%s\n' 'Select the client to onboard for this project:' '  1) Codex' '  2) ZCode' '  3) Both'
        printf 'Enter number or name (q to quit): '
        read -r client_choice
        case "$client_choice" in
          1|[cC]odex) wire_codex=1 ;;
          2|[zZ][cC]ode) wire_zcode=1 ;;
          3|[bB]oth) wire_codex=1; wire_zcode=1 ;;
          [qQ]) note 'Cancelled: no client selected; nothing was written.'; return 0 ;;
          *) die "Invalid selection: $client_choice" ;;
        esac
      else
        die '未检测到 .codex/ 或 .zcode/ 客户端目录；请用 --client codex|zcode|both 指定要接入的客户端'
      fi
    fi
  fi
  ...
  if [ "$wire_codex" -eq 1 ]; then
    ...
    [ ! -e "$codex_dir" ] || [ -d "$codex_dir" ] || die ".codex 必须是目录"
  fi
  if [ "$wire_zcode" -eq 1 ]; then
    [ ! -L "$zcode_dir" ] || die ".zcode 不能是 symlink"
    [ ! -e "$zcode_dir" ] || [ -d "$zcode_dir" ] || die ".zcode 必须是目录"
  fi
  assert_plain_file_or_absent "$codex_config"
  assert_plain_file_or_absent "$zcode_config"
...
- 安装、配置修复、初始化或修复索引、健康检查、升级审查和回滚使用全局 `$agent-toolchain`；不得在日常开发中自行安装、升级、重配或维护工具链。'
  ...
  if [ "$wire_codex" -eq 1 ]; then
    if [ -f "$codex_config" ] && rg -q '^\[mcp_servers\.codegraph\][[:space:]]*$' "$codex_config"; then
      main_body=$(toml_table_body "$codex_config" '[mcp_servers.codegraph]') || die ".codex/config.toml 的 CodeGraph 主表重复或无效"
      env_body=$(toml_table_body "$codex_config" '[mcp_servers.codegraph.env]') || die ".codex/config.toml 的 CodeGraph env 表缺失、重复或无效"
    ...
    elif [ -f "$codex_config" ] && rg -q '^\[mcp_servers\.codegraph\.' "$codex_config"; then
...
if not isinstance(data, dict):
    raise SystemExit('.zcode/config.json 顶层必须是 JSON 对象')
mcp = data.setdefault('mcp', {})
if not isinstance(mcp, dict):
    raise SystemExit('.zcode/config.json 的 mcp 必须是对象')
servers = mcp.setdefault('servers', {})
if not isinstance(servers, dict):
    raise SystemExit('.zcode/config.json 的 mcp.servers 必须是对象')
if 'codegraph' in servers:
    server = servers['codegraph']
    env = server.get('env') if isinstance(server, dict) else None
    if (not isinstance(server, dict)
            or server.get('command') != 'codegraph'
            or list(server.get('args') or []) != ['serve', '--mcp']
            or not isinstance(env, dict)
            or env.get('CODEGRAPH_TELEMETRY') != '0'
            or env.get('CODEGRAPH_NO_UPDATE_CHECK') != '1'
            or env.get('DO_NOT_TRACK') != '1'):
        raise SystemExit('.zcode/config.json 的 CodeGraph 配置冲突')
else:
    servers['codegraph'] = {
        'type': 'stdio',
        'command': 'codegraph',
...
PYEOF
  ...
  if [ "$wire_codex" -eq 1 ]; then
    ...
    [ "$codex_config_needs_write" -eq 0 ] || append_project_text "$codex_config" '[mcp_servers.codegraph]
command = "codegraph"
args = [
    "serve",
    "--mcp",
...
DO_NOT_TRACK = "1"'
  ...
  if [ "$wire_zcode" -eq 1 ]; then
    ...
    if [ "$zcode_config_needs_write" -eq 1 ]; then
      printf '%s\n' '{
  "mcp": {
    "servers": {
      "codegraph": {
