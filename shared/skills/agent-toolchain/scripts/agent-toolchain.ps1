...
function Show-Usage {
  @'
Usage:
  agent-toolchain.ps1 configure --project PATH [--client codex|zcode|both]
...
function Parse-Arguments {
  ...
  while ($index -lt $DriverArgs.Count) {
    ...
    switch ($argument) {
      ...
      '--client' {
        if ($index + 1 -ge $DriverArgs.Count) { Fail '--client 缺少取值' }
        $script:Client = $DriverArgs[$index + 1]
        if ($script:Client -notin @('codex', 'zcode', 'both')) { Fail "--client 取值无效：$($script:Client)（可用：codex|zcode|both）" }
        $index += 2
      }
  ...
  if ($script:Client -and $script:Action -ne 'configure') { Fail '--client 仅适用于 configure' }
}
...
function Configure-Project {
  ...
  $zcodeDirectory = Join-Path $script:Project '.zcode'
  $zcodeConfigPath = Join-Path $zcodeDirectory 'config.json'
  $agentsPath = Join-Path $script:Project 'AGENTS.md'
  $ignorePath = Join-Path $script:Project '.gitignore'

  $wireCodex = $false; $wireZcode = $false
  if ($script:Client) {
    $wireCodex = ($script:Client -eq 'codex') -or ($script:Client -eq 'both')
    $wireZcode = ($script:Client -eq 'zcode') -or ($script:Client -eq 'both')
  } else {
    $wireCodex = Test-Path -LiteralPath $codexDirectory
    $wireZcode = Test-Path -LiteralPath $zcodeDirectory
    if (-not $wireCodex -and -not $wireZcode) {
      if ([Console]::IsInputRedirected) { Fail '未检测到 .codex/ 或 .zcode/ 客户端目录；请用 --client codex|zcode|both 指定要接入的客户端' }
      Write-Host 'Select the client to onboard for this project:'
      Write-Host '  1) Codex'
      Write-Host '  2) ZCode'
      Write-Host '  3) Both'
      ...
      switch ($choice) {
        '1' { $wireCodex = $true }
        '2' { $wireZcode = $true }
        '3' { $wireCodex = $true; $wireZcode = $true }
        'codex' { $wireCodex = $true }
        'zcode' { $wireZcode = $true }
        'both' { $wireCodex = $true; $wireZcode = $true }
        'q' { Note 'Cancelled: no client selected; nothing was written.'; return }
        default { Fail "Invalid selection: $choice" }
      }
    }
  }

  if ($wireCodex -and (Test-Path -LiteralPath $codexDirectory)) {
    $codexItem = Get-Item -LiteralPath $codexDirectory -Force
    if (($codexItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 -or -not $codexItem.PSIsContainer) { Fail '.codex 必须是非链接目录' }
  }
  if ($wireZcode -and (Test-Path -LiteralPath $zcodeDirectory)) {
    $zcodeItem = Get-Item -LiteralPath $zcodeDirectory -Force
    if (($zcodeItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 -or -not $zcodeItem.PSIsContainer) { Fail '.zcode 必须是非链接目录' }
  }
  Assert-PlainFileOrAbsent $codexConfigPath
  Assert-PlainFileOrAbsent $zcodeConfigPath
  ...
  if ($wireCodex) {
    $configText = if (Test-Path -LiteralPath $codexConfigPath) { Get-Content -LiteralPath $codexConfigPath -Raw } else { '' }
    if ($configText -match '(?m)^\[mcp_servers\.codegraph\]\s*$') {
      $mainBody = Get-TomlTableBody $codexConfigPath '[mcp_servers.codegraph]'
      $envBody = Get-TomlTableBody $codexConfigPath '[mcp_servers.codegraph.env]'
  ...
  $zcodeJson = $null
  if ($wireZcode) {
    if (Test-Path -LiteralPath $zcodeConfigPath) {
      try { $zcodeJson = Get-Content -LiteralPath $zcodeConfigPath -Raw | ConvertFrom-Json -AsHashtable } catch { Fail '.zcode/config.json 不是有效的 JSON' }
    } else {
      $zcodeJson = [ordered]@{}
    }
    if ($zcodeJson -isnot [System.Collections.IDictionary]) { Fail '.zcode/config.json 顶层必须是 JSON 对象' }
    if (-not $zcodeJson.Contains('mcp')) { $zcodeJson['mcp'] = [ordered]@{} }
    elseif ($zcodeJson['mcp'] -isnot [System.Collections.IDictionary]) { Fail '.zcode/config.json 的 mcp 必须是对象' }
    if (-not $zcodeJson['mcp'].Contains('servers')) { $zcodeJson['mcp']['servers'] = [ordered]@{} }
    elseif ($zcodeJson['mcp']['servers'] -isnot [System.Collections.IDictionary]) { Fail '.zcode/config.json 的 mcp.servers 必须是对象' }
    $zcodeServers = $zcodeJson['mcp']['servers']
    if ($zcodeServers.Contains('codegraph')) {
      $existing = $zcodeServers['codegraph']
      $existingEnv = $null
      if ($existing -is [System.Collections.IDictionary]) { $existingEnv = $existing['env'] }
      if ($existing -isnot [System.Collections.IDictionary] -or $existing['command'] -ne 'codegraph' -or (@($existing['args']) -join ',') -ne 'serve,--mcp' -or $existingEnv -isnot [System.Collections.IDictionary] -or $existingEnv['CODEGRAPH_TELEMETRY'] -ne '0' -or $existingEnv['CODEGRAPH_NO_UPDATE_CHECK'] -ne '1' -or $existingEnv['DO_NOT_TRACK'] -ne '1') { Fail '.zcode/config.json 的 CodeGraph 配置冲突' }
    } else {
      $zcodeServers['codegraph'] = [ordered]@{
        type = 'stdio'
        command = 'codegraph'
        args = @('serve', '--mcp')
        env = [ordered]@{ CODEGRAPH_TELEMETRY = '0'; CODEGRAPH_NO_UPDATE_CHECK = '1'; DO_NOT_TRACK = '1' }
      }
...
'@
  ...
  if ($wireZcode) {
    ...
    if ($needsZcodeConfig) {
      [IO.File]::WriteAllText($zcodeConfigPath, "$($zcodeJson | ConvertTo-Json -Depth 10)`n", [Text.UTF8Encoding]::new($false))
    }
  }
  if ($needsAgents) {
