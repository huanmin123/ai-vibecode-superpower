...
description: 通过本地 imagegen CLI 和模型 gpt-image-2 生成图像；将安装时已解析的 `<CODEX_HOME>/auth.json` 与 `<CODEX_HOME>/config.toml` 中的凭据和提供方配置桥接为 `OPENAI_API_KEY`、`OPENAI_BASE_URL`。用户明确要求 gpt-image-2 图像生成、指出官方/API 示例误用非图像模型或错误端点、要求复现既有 Codex 鉴权/配置图像生成流程，或需要使用当前 Codex 提供方而非内置 `image_gen` 工具输出图像时使用。
---
...
3. 运行辅助脚本。安装器会把本文的 `<CODEX_HOME>` 替换为本机绝对路径；在已安装版本中直接使用该路径，不得自行拼接或把占位符作为字面路径传给工具。

Windows/macOS/Linux 上的 PowerShell 7：

```powershell
$skill = '<CODEX_HOME>\skills\gpt-image-2-cli'
if (-not (Test-Path -LiteralPath $skill -PathType Container)) { throw '未找到已安装的 gpt-image-2-cli skill。' }
...
```bash
skill="<CODEX_HOME>/skills/gpt-image-2-cli"
[ -d "$skill" ] || { printf '%s\n' '未找到已安装的 gpt-image-2-cli skill。' >&2; exit 1; }
...
- 否则从已安装的 `<CODEX_HOME>/auth.json` 读取 `OPENAI_API_KEY`。
- 从已安装的 `<CODEX_HOME>/config.toml` 读取 `model_provider` 和对应提供方的 `base_url`，将其导出为子 CLI 进程使用的 `OPENAI_BASE_URL`。
- 使用平台原生的路径拼接；无需 Windows 专用分隔符或机器特定的主目录。
- 调用随附的系统脚本：

```text
<CODEX_HOME>/skills/.system/imagegen/scripts/image_gen.py generate --model gpt-image-2
```
...
-CodexHome <path>  # 可选，覆盖安装时写入的绝对 Codex home
...
- 辅助脚本找不到 `OPENAI_API_KEY` 时，检查 `<CODEX_HOME>/auth.json` 或在本地设置环境变量。绝不要求用户把完整 key 粘贴到对话中。
